...
@router_departments.get("/", response_model=Any)
async def get_departments_endpoint(
    ...
    # 1. Если передан department_id - возвращаем департамент с отделами и группами
    ...
    # 2. Если передана abbreviation - ищем по точному совпадению во всех сущностях
    if abbreviation is not None:
        abbreviation_upper = abbreviation.upper()

        # Поиск в департаментах
        result = await db.execute(select(Department).where(Department.abbreviation == abbreviation_upper))
        ...
        # Поиск в отделах
        result = await db.execute(select(Division).where(Division.abbreviation == abbreviation_upper))
        item = result.scalar_one_or_none()
        if item:
            return SearchByAbbreviationResponse(
        ...
        # Поиск в группах
        result = await db.execute(select(Group).where(Group.abbreviation == abbreviation_upper))
        item = result.scalar_one_or_none()
        if item:
            return SearchByAbbreviationResponse(
                entity_type="group",
    ...
    # 3. Иначе - стандартная выдача списка с фильтрацией по name
    departments = await get_departments_list(db, skip, limit, name, None)
    # Явно валидируем объекты SQLAlchemy в Pydantic-схему, чтобы FastAPI мог их сериализовать
    return [DepartmentShortResponse.model_validate(d) for d in departments]
