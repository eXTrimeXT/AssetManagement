...
async def bulk_enrich_assets(db: AsyncSession, assets: list) -> None:
    ...
    # Кэшируем иерархии подразделений (вызов тяжелой функции только для уникальных GUID)
    ...
    # Обогащаем данные в памяти, используя готовые кэши
    for asset in assets:
        if asset.users:
