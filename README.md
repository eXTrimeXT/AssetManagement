...
Система уведомлений построена на реактивной архитектуре с использованием триггеров базы данных PostgreSQL и технологии Server-Sent Events (SSE) для мгновенной доставки событий на фронтенд без необходимости постоянного опроса (polling).

#### Статусы уведомлений
Каждое уведомление имеет один из трех статусов (`status`):
*   `unread` (Не прочитано) — уведомление создано, но пользователь еще не взаимодействовал с ним.
*   `read` (Прочитано) — пользователь ознакомился с уведомлением (через API или интерфейс).
*   `declined` (Отклонено) — пользователь отклонил действие, связанное с уведомлением (например, отказался от назначения ответственным).

#### Типы событий и логика создания
Уведомления создаются автоматически при изменении связанных сущностей. Основные типы событий (`event_type`):

| Тип события (`event_type`) | Когда создается                                                                                                                   |
|:---------------------------|:----------------------------------------------------------------------------------------------------------------------------------|
| `assigned_responsible`     | При назначении сотрудника **ответственным** за актив (создание записи в `asset_assignments`).                                     |
| `assigned_user`            | При назначении сотрудника **пользователем** актива.                                                                               |
| `unassigned_responsible`   | При снятии назначения ответственного (удаление записи или установка `end_date`).                                                  |
| `unassigned_user`          | При снятии назначения пользователя.                                                                                               |
| `responsible_declined`     | **Ответное уведомление:** создается инициатору, когда назначенный ответственный отклоняет назначение.                             |
| `user_declined`            | **Ответное уведомление:** создается инициатору, когда назначенный пользователь отклоняет назначение.                              |
| `write_off_requested`      | При создании новой заявки на списание актива.                                                                                     |
| `write_off_approved`       | При утверждении заявки на списание администратором.                                                                               |
| `write_off_rejected`       | При отклонении заявки на списание администратором.                                                                                |
| `service_due`              | Создается фоновой задачей планировщика (`check_service_assets`), если подошел срок еженедельной проверки или обслуживания актива. |
| `inventory_started`        | Создается когда была создана новая сессия инвентаризации.                                                                         |
| `inventory_discrepancy`    | Когда обнаружено расхождение по активу.                                                                                           |
| `inventory_completed`      | Создается когда сессия инвентаризации завершена.                                                                                  |

#### Архитектура SSE (Server-Sent Events)
Доставка уведомлений в реальном времени реализована по следующей цепочке:

1. **Триггер базы данных:** При любом изменении в таблице `notifications` (`INSERT`, `UPDATE` или `DELETE`) срабатывает триггер `trigger_notification_change`. Он вызывает функцию `notify_notification_change()`, которая отправляет минимальный сигнал через `pg_notify('notification_channel', ...)` с указанием `employee_id` получателя.
2. **Фоновый слушатель (Listener):** В `main.py` запущена асинхронная задача `db_notification_listener`. Она использует **нативное подключение `asyncpg`** (в обход пула SQLAlchemy) для прослушивания канала `notification_channel`. Это гарантирует отсутствие блокировок и высокую производительность.
3. **Менеджер подключений (`NotificationManager`):** Хранит **множество очередей** (`Set[asyncio.Queue]`) для каждого `employee_id`. Это критически важная особенность, которая позволяет одному пользователю открыть систему в **нескольких вкладках браузера одновременно**, и все вкладки будут корректно получать обновления, не перезаписывая друг друга.
4. **Эндпоинт `/api/notifications/stream`:**
    * При первом подключении клиент мгновенно получает **полный актуальный список** своих уведомлений (историю), сериализованный через схему `NotificationResponse` (включая вычисляемые поля: `event_type_ru`, `asset_name`, `initiator_full_name`).
    * При получении сигнала об изменении из очереди, сервер заново запрашивает полный список из БД и отправляет его клиенту целиком.
    * Такой подход ("полная замена состояния") гарантирует 100% синхронизацию данных между сервером и всеми открытыми вкладками клиента, исключая рассинхронизацию при одновременном прочтении или удалении уведомлений в разных вкладках.


#### **Для создания функций в БД можно использовать файлы миграций:**
1) `68755060ef51_create_notify_new_notification_NEED.py` - создает функцию, которая нужна для получения потока уведомлений (не обязательно применять миграцию (старая версия))
2) `260cdae63265_create_notify_notification_change_NEED.py` - создает триггерную функцию, которая при любом изменении таблицы `notifications` бросает данные в эндпоинт `api/notifications/stream`

#### Основные API эндпоинты
| Метод    | URL                                  | Описание                                                                                       |
|:---------|:-------------------------------------|:-----------------------------------------------------------------------------------------------|
| `GET`    | `/api/notifications/my`              | Получить список уведомлений с пагинацией, фильтрацией по статусу и активу.                     |
| `GET`    | `/api/notifications/stream`          | Установить SSE-соединение для получения обновлений в реальном времени.                         |
| `GET`    | `/api/notifications/my/unread-count` | Получить только количество непрочитанных уведомлений.                                          |
| `PATCH`  | `/api/notifications/{id}/read`       | Пометить конкретное уведомление как прочитанное.                                               |
| `PATCH`  | `/api/notifications/my/read-all`     | Пометить все уведомления пользователя как прочитанные.                                         |
| `POST`   | `/api/notifications/{id}/decline`    | Отклонить назначение (меняет статус на `declined` и закрывает привязку в `asset_assignments`). |
| `DELETE` | `/api/notifications/{id}`            | Полностью удалить уведомление.                                                                 |
| `DELETE` | `/api/notifications/my/clear-read`   | Массовое удаление всех прочитанных уведомлений.                                                |
