...
class Asset(Base):
    ...
    cost_center_name_from = Column(String(50), nullable=True)       # Название ответственного департамента
    cost_center_shortname_from = Column(String(12), nullable=True)  # Аббревиатура ответственного департамента

    cost_center_code = Column(String(10), nullable=True)        # Код департамента владельца
    cost_center_name = Column(String(50), nullable=True)        # Название департамента владельца
    ...
    def users(self) -> List[AssetUserFullResponse]:
        result = []
        for a in self.assignments:
            if a.end_date is not None or a.assignment_type != "user":
                continue
            emp = a.employee
            if not emp:
                continue
    ...
    def responsible_users(self) -> List[AssetUserFullResponse]:
        result = []
        for a in self.assignments:
            if a.end_date is not None or a.assignment_type != "responsible":
                continue
            emp = a.employee
            if not emp:
                continue
    ...
    def serving_users(self) -> List[AssetUserFullResponse]:
        result = []
        for a in self.assignments:
            if a.end_date is not None or a.assignment_type != "serving":
                continue
            emp = a.employee
            if not emp:
                continue
