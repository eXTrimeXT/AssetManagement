...
async def get_asset_type_en_name(
        db: AsyncSession,
        model_id: Optional[int],
        asset_type_id: Optional[int]
) -> Optional[str]:
    """
    Определяет en_name типа актива.
    Приоритет: model_id → asset_type_id.
    """
    # 1. Если есть model_id — идём через модель
    if model_id is not None:
        model = await get_asset_model_by_id(db, model_id)
        if model and model.asset_type:
            return model.asset_type.en_name

    # 2. Если есть asset_type_id — берём напрямую
    if asset_type_id is not None:
        asset_type = await get_asset_type_by_id(db, asset_type_id)
        if asset_type:
            return asset_type.en_name
...
async def get_asset(
    ...
    en_name = await get_asset_type_en_name(db, obj.model_id, obj.asset_type_id)
    if en_name:
...
async def update_asset_endpoint(
    ...
    final_model_id = data.model_id if data.model_id is not None else obj.model_id
    final_asset_type_id = data.asset_type_id if data.asset_type_id is not None else obj.asset_type_id

    en_name = await get_asset_type_en_name(db, final_model_id, final_asset_type_id)
    if en_name:
        has_perm = await check_permission(request, en_name, "write")
        if not has_perm:
            raise HTTPException(
        ...
        raise HTTPException(status_code=400, detail="Не удалось определить тип актива после обновления")
...
async def get_asset_children_endpoint(
    ...
    en_name = await get_asset_type_en_name(db, parent.model_id, parent.asset_type_id)
    if en_name:
        has_perm = await check_permission(request, en_name, "read")
        if not has_perm:
            raise HTTPException(
