...
async def sync_zup_data_job():
    ...
    """
    logger.info("[1C SYNC] Запуск запланированной синхронизации данных из 1С-ЗУП...")
    try:
        # Создаем сессию вручную, так как мы находимся вне контекста HTTP-запроса
        async with async_session() as db:
            stats = await sync_all_data(db)
            logger.info(f"[1C SYNC] Синхронизация 1С-ЗУП успешно завершена. Статистика: {stats}")
    except Exception as e:
        logger.error(f"[1C SYNC] Критическая ошибка при синхронизации данных из 1С-ЗУП: {e}", exc_info=True)