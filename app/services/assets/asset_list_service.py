...
#
#         if local_orm_items:
#             # "Запекаем" в Pydantic для безопасной сериализации
...
#         # else:
#             # Если в локальной БД нет, возвращаем пустой результат.
#             # (Поиск по material_id в SAP API не поддерживается параметрами фильтрации)
#             return _build_paginated_response([], total=0, page=1, page_size=1)
#
#     # === Стандартная логика пагинации и слияния (для обычных поисков) ===
...
async def _fetch_and_merge_sap_assets(
    ...
    Возвращает:
        virtual_assets — список виртуальных активов (после фильтрации).
        sap_total — общее количество записей в SAP под фильтрами (из ответа API).
    """
    try:
        sap_response = await _fetch_sap_materials(
            page=1,
        ...
        # Фильтрация дубликатов в Python
        filtered_sap_items = []
        employee_ids = set()
        department_codes = set()
        ...
        for sap_item in filtered_sap_items:
            virtual_asset = _build_virtual_asset(sap_item, employees_map, departments_map)
            if asset_type_id is None or virtual_asset.get("asset_type_id") == asset_type_id:
                virtual_assets.append(virtual_asset)
